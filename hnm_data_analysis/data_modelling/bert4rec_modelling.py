...
# Writing a self-contained BERT4Rec module tailored for Polars-based preprocessing, 
# with UK spellings in identifiers, comments, and docstrings. 
# The file will be saved to /mnt/data/bert4rec_polars.py

from pathlib import Path
"""
BERT4Rec for implicit product recommendation (Polars + PyTorch)

This module provides a clean, testable implementation of BERT4Rec with a Polars
data pipeline. It is designed to train on transaction sequences and evaluate
next-item prediction with UK spellings in identifiers, comments, and docstrings.
...
- Lightweight Transformer encoder with tied input/output embeddings
- Time-based train/validation/test splits to avoid leakage
- Recall@K and NDCG@K evaluation for next-item prediction
- Sensible defaults; minimal external dependencies (PyTorch + Polars)
...
def prepare_sequences_with_polars(
    ...
    -------
    PreparedData
        Sequences of token ids per user, token registry, and auxiliar info.
    """
